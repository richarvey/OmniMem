...
| **Memories** | Browse all memories with namespace, state, project, and licence filters (`?licence=unknown` is the queue of records nobody has classified yet — they wear an **unclassified** pill), plus inline deprioritise/delete actions on each row. A recall-heat rule on each row fades with time since the memory last surfaced in recall. Paginated, htmx-powered. The Preferences, Articles, and Learned Knowledge sidebar entries are filtered views of this page |
| **Search** | Semantic search using the full recall pipeline. Abandoned warnings highlighted |
| **Detail** | Full memory content, metadata, tags, experience data, contradictions. Lifecycle action buttons, inline tag editing (comma-separated, same validation as the `retag` tool), and a redistribution-rights form: pick the licence class, add an optional note, save — the same engine as the `set_licence` tool, so extracted facts follow and a pre-filled note is dropped when the class changes |
| **Create** | Store a new memory with duplicate detection shown inline. A licence selector says whether the content may be redistributed; left blank it takes the namespace default (own work, or unknown for knowledge) |
