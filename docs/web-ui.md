...
| **Memories** | Browse all memories with namespace, state, project, licence, and provenance filters (`?licence=unknown` is the queue of records nobody has classified yet — they wear an **unclassified** pill; the filter resolves an unstamped record the way recall does, so the queue is complete even before a restart backfills), plus inline deprioritise/delete actions on each row. A recall-heat rule on each row fades with time since the memory last surfaced in recall. Paginated, htmx-powered. The Preferences, Articles, and Learned Knowledge sidebar entries are filtered views of this page |
| **Search** | Semantic search using the full recall pipeline. Abandoned warnings highlighted |
| **Detail** | Full memory content, metadata, tags, experience data, contradictions. Lifecycle action buttons, inline tag editing (comma-separated, same validation as the `retag` tool), and a redistribution-rights form: pick the licence class, add an optional note, save — the same engine as the `set_licence` tool, so extracted facts follow and a pre-filled note is dropped when the class changes — and a provenance form beside it (asserted / concluded / retrieved), same engine as `set_provenance` |
| **Create** | Store a new memory with duplicate detection shown inline. A licence selector says whether the content may be redistributed; left blank it takes the namespace default (own work, or unknown for knowledge). A provenance selector does the same for where it came from |
| **Projects** | List, view, edit, and create project contexts. Each project declares its work-type domains, shown as pills on both the list and the detail view and filterable from a chip bar above the table (`/projects?domain=python`); a domain that already has a compiled skill links straight to it. The edit form takes a comma-separated list with a datalist of every domain in use across projects and skills, and a **Suggest from stack & memories** button drafts one from the project's stack field and its recurring tags — showing the evidence for each and filling the field without saving, so you still press Save |
| **Skills** | Browse compiled skills: rules with reinforcement counts and source citations, the full SKILL.md body, load counters, and the source manifest linking back to the memories each skill was compiled from. The New Skill modal compiles a draft for a domain through the same propose-and-accept gate as the MCP `compile_skill` tool — you review the draft in place and nothing is written until you accept it. Skills can be deleted (with confirmation) but never edited: to change one, update the underlying memories and recompile. Export downloads a skill together with its source memories — and any RSS feeds that influence its domain — as a checksummed zip bundle; Import validates such a bundle, previews exactly what would be added, and only writes on confirm — strictly additive, so existing skills, memories, and feeds on the receiving instance are never overwritten (a feed already in the reading list at most gains the influence entry it was missing) |
...
| **RSS Feeds** | Add, edit, and delete feeds, or download/upload feeds.yml whole — the worker picks changes up automatically. Each feed can be tied to compiled skills with an influence score (1–10): recompiles of those skills then pull the feed's latest articles into their Feed watch section, the score setting how many articles the feed contributes. Each feed also declares the licence stamped on every article it ingests — open, restricted, or left undeclared, which ingests as unknown and shows as **unclassified** in the list |
