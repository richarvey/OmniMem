...
The Python tree was the reference implementation through phase 8 and has now been removed from this branch (6.x carries on in the `v6.x` branches). The golden fixtures captured from it stay. Two carry the script that took them (`crates/omnimem-embed/tests/fixtures/generate_reference.py` and `crates/omnimem-mcp/tests/fixtures/oauth_golden.py`), which run against a `v6.7.x` checkout; `crates/omnimem-engine/tests/fixtures/skills_golden.json` does not.
...
- **Tray / menu bar icon** (`tray-icon` + `muda`): present while OmniMem runs. The icon shows state (running, starting, needs attention) and the menu offers:
  - status line: memory count (the address is behind Copy MCP URL)
...
1. **MCP tools**: the same 48 names, parameters and defaults, and results in the same JSON shape, including `_compact()` dropping empty values. The descriptions started as verbatim copies of the 6.x docstrings and were later trimmed of their `Args:` blocks, which duplicated the schema
...
3. **Recall scoring**: floor, weak band, temporal boost and fact collapse behave exactly as `memory/recall.py`; recency decay and the abandoned experience weight were deliberately changed after the port. Originally: floor, weak band, recency decay, experience weight, temporal boost, reinstate 0.6, fact collapse and ordering exactly as `memory/recall.py`
...
6. **Configuration**: every 6.x environment variable honoured with its default, except the Valkey and web UI ones (`WEB_UI_*`) and `EMBEDDING_BACKEND`, which are dropped. Settings can also be changed in the desktop app's settings panel
...
|---|---|---|
| **0. Foundation** | Workspace, `omnimem-core` (keys, content hash), `omnimem-embed` | Rust vectors match `reference_vectors.json` within 1e-4 |
