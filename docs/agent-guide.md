...
The full OmniMem usage guide. The MCP server sends a condensed version of
this as its `instructions` block; the long form lives here so it costs no
tokens on the wire. Keep the two in step when behaviour changes.
