...
**Memory A**, the first report episode:
...
**Memory B**, a second episode reinforcing the same lessons in different words:
...
**Memory D**, the gate victim (one source, no bless, so it should be *excluded*, and the compile output will say so):
...
The skill also appears on the Skills page of the desktop app's settings panel, with each rule linking back to its source memory. Good for the "provenance" beat.
...
1. **Delete the skill** in the settings panel: Skills → moo-reports → Delete (with confirmation). There is deliberately no MCP delete for skills.
...
Quick verification that it's all gone: `find_skills("moo")` returns nothing, `recall("moo report")` returns nothing, and the settings panel's Memories page filtered by the `demo` tag is empty.
