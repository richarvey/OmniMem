...
- **Fewer results than you asked for is a real answer.** `top_k` is a ceiling, not a quota: anything below the relevance floor (`RECALL_MIN_SCORE`, default 0.4) is dropped rather than padding the list. An empty return means nothing stored is relevant — say so and move on. Do not go looking for a connection between the query and a result that came back; if it came back, it already cleared the bar
