...
## [7.0.0] - Unreleased
### Changed
- **OmniMem is being ported to a single Rust binary** (in progress): one process replacing the four containers, with its own vector store (SQLite, vectors searched in memory), the MCP server, RSS ingestion and the web UI, where settings will live. The plan, the decisions behind it and the compatibility contract are in `docs/rust-port-plan.md`. The Python tree stays as the reference implementation until the Rust binary reaches parity. First phase landed: a Cargo workspace beside the Python, `omnimem-core` with the v7 content hash and key types, and `omnimem-embed`, an ONNX Runtime engine that reproduces the Python 6.7 engine's vectors (checked against full reference vectors the Python engine wrote, covering accents, CJK, emoji, empty text, truncation past 256 tokens and mixed-length batches), so a 6.x store will import without its recall or skill thresholds moving
