...
- **One engine behind both classification tools**: `set_licence`, `set_provenance` and the two web forms all go through a shared lineage engine that stamps the memories, cascades to their extracted facts (by `enriched_from` and by `source_doc_id`, so batch-enriched documents are covered), and deliberately leaves `updated_at` alone — the tools and routes in front of it refuse skills and non-memory keys before anything reaches it — the skill compiler reads a bumped timestamp as "source changed". The vocabulary lives in one constant because the v7 Mycelium `ClusterSummary` carries a `provenance_class` that must use the same values; a mismatch there would be silent until cluster time
- **Reads never depend on backfill timing**: recall, `recall_detail`, `recent_knowledge`, the briefing's new-knowledge list and `explain_memory` resolve a missing `provenance` or `licence` at read time with the backfill's own rules — an article is `retrieved`/`unknown`, a context entry `asserted`, everything else the namespace default — so a record stamped by a worker image that predates the field during a rolling upgrade still reports something honest (and an unclassified article still raises the `licence_notice`) rather than silently omitting the key. An out-of-vocabulary stored value reads as absent, and skill import falls back to the namespace default for one, so a filter can never fail to match what recall reports
...
- **Test suite grew to 1762 tests**, with every `memory/` and `tools/` module at 100% line coverage
...
- **Test suite grew to 1694 tests**, with every module touched by this release at 100% line coverage
...
- **Test suite grew to 1544 tests**, with every module touched by this release at 100% line coverage
### Fixed
