...
- **The settings panel** (Rust port, phase 7, in progress): the web UI's pages move into the desktop app's settings window, rendered by the app itself from the same templates and served to the window over `omnimem://`, so nothing is listening on a port. Ported so far: the dashboard, the memories list with its filters, pages and the Preferences, Articles and Learned Knowledge views, the memory detail page with its tag, licence and provenance forms, deprioritise, archive, reinstate and delete, the create form with its duplicate check, semantic search, projects (with domain filtering, suggestions, editing and bulk deprioritise and reinstate), experience with the graveyard, and compiled skills with the New Skill review, delete, export and import. An exported bundle is saved straight into your Downloads folder, and the skills page tells you the file's name. The forms go through the same engine calls as the MCP tools. Pages still to move show a note saying so, and while the services are starting the window says that too, or why they stopped. The version marker in the sidebar still checks for a newer release, and never shows on a development build
